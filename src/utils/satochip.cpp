...
static std::vector<unsigned char> normalize_ecdsa_signature(
  ...
  if (!secp256k1_ecdsa_signature_serialize_der(secp_ctx, out.data(), &out_size,
                                               &sig)) {
    throw std::runtime_error(
        "[Satochip] secp256k1_ecdsa_signature_serialize_compact error.");
  }
  out.resize(out_size);
...
static std::optional<SatochipMuSig2Setup> get_musig2_setup(
  ...
  if (sigversion == SigVersion::TAPROOT && merkle_root != nullptr) {
    ...
    setup.tweaks.emplace_back(
        script_xonly.ComputeTapTweakHash(tap_merkle_root), true);
...
static std::vector<unsigned char> get_musig2_keyagg_coefficient(
  ...
  auto second_distinct = std::find_if(
      std::next(participants.begin()), participants.end(),
      [&](const CPubKey &participant) { return participant != participants[0]; });
  ...
  coefficient_data.insert(coefficient_data.end(), pks_hash.begin(),
                          pks_hash.end());
  coefficient_data.insert(coefficient_data.end(), pubkey.begin(),
                          pubkey.end());
  uint256 coefficient = tagged_hash_bytes(
      "KeyAgg coefficient",
      std::span<const unsigned char>(coefficient_data.data(),
                                     coefficient_data.size()));
...
static SatochipMuSig2SignData get_satochip_musig2_sign_data(
  ...
  if (!MuSig2AggregatePubkeys(
          participants, data.keyagg_cache,
...
static bool verify_musig2_partial_sig(
  ...
  return secp256k1_musig_partial_sig_verify(
             secp_ctx, &secp_partial_sig, &secp_pubnonce, &secp_pubkey,
             &keyagg_cache, &session) != 0;
}

static CExtPubKey SatochipGetExtPubKey(
    CardBip32GetExtendedKeyFn cardBip32GetExtendedKeyFn, std::string path,
  ...
  if (card_bip32.size() < 2 || parent.size() < 2) {
...
std::string SatochipGetDescriptor(
    ...
    std::string path, bool is_testnet) {
  std::vector<uint32_t> parsed_path;
  std::replace(path.begin(), path.end(), 'h', '\'');
  if (!ParseHDKeypath(path, parsed_path)) {
    throw std::runtime_error("[Satochip] invalid path.");
  ...
  auto xpub = EncodeBase58Check(encoded);
  return strprintf("[%s%s]%s", HexStr(master_fingerprint),
                   FormatHDKeypath(parsed_path), xpub);
}
...
std::string SatochipSignPsbt(const SatochipSignPsbtParams &params,
  ...
  auto sign_taproot_input = [&](int index, PSBTInput &input,
    ...
    auto sign_schnorr_hash = [&](const XOnlyPubKey &verify_key,
      ...
      auto signature = params.cardSignSchnorrHashFn(
          {hash.begin(), hash.end()}, params.chalresponse);
    ...
    auto process_musig2_session =
        [&](const CPubKey &my_pubkey, const CPubKey &aggregate_pubkey,
            const std::vector<CPubKey> &participants,
            const XOnlyPubKey &script_xonly, const uint256 *merkle_root,
            const uint256 *leaf_hash, SigVersion sigversion,
            bool key_path) {
          ...
          if (!key_path &&
              input.m_tap_script_sigs.count(
                  std::make_pair(XOnlyPubKey(setup->script_pubkey),
                                 lookup_leaf_hash))) {
            return true;
          }
          ...
          auto aggregate_sig = [&]() {
            ...
            if (!all_musig2_pubnonces_present(pubnonce_it->second,
                                              participants) ||
          ...
          if (all_musig2_pubnonces_present(pubnonces, participants)) {
            ...
            auto sign_data = get_satochip_musig2_sign_data(
                my_pubkey, aggregate_pubkey, participants, pubnonces,
                setup->tweaks, hash);
            auto partial_sig = params.cardMusig2SignFn(
                0xff, *secnonce, sign_data.b, sign_data.ea,
                sign_data.r_has_even_y, sign_data.ggacc_is_1);
            if (partial_sig.size() != 32 ||
                !verify_musig2_partial_sig(my_pubkey, partial_sig,
                                           pubnonces.at(my_pubkey),
                                           sign_data.keyagg_cache,
                                           sign_data.session)) {
              throw NunchukException(NunchukException::INVALID_SIGNATURE,
          ...
          if (!pubnonces.count(my_pubkey)) {
            if (!params.cardMusig2GenerateNonceFn) {
              throw std::runtime_error(
                  "[Satochip] missing MuSig2 nonce callback.");
            }
            if (!saveSecNonceFn) {
              throw std::runtime_error(
                  "[Satochip] missing MuSig2 secnonce storage callback.");
            ...
            auto nonce_resp = params.cardMusig2GenerateNonceFn(
                0xff, aggregate_xonly_bytes, {hash.begin(), hash.end()},
                extra);
            ...
            if (leaf_hashes.empty()) {
              ...
              if (!input.m_tap_internal_key.IsNull()) {
                key_path_processed = process_musig2_session(
                    ...
                    input.m_tap_internal_key, &input.m_tap_merkle_root,
                    nullptr, SigVersion::TAPROOT, true);
              }
    ...
    for (const auto &[xonly_pub, leaf_pair] : input.m_tap_bip32_paths) {
      ...
      const bool signs_internal_key =
          key_path_candidate && !input.m_tap_internal_key.IsNull() &&
      ...
      if (signs_internal_key || signs_output_key) {
        ...
        if (signs_internal_key) {
          if (!params.cardTaprootTweakPrivateKeyFn) {
            throw std::runtime_error(
                "[Satochip] missing taproot tweak callback.");
          }
          const uint256 *merkle_root =
              input.m_tap_merkle_root.IsNull() ? nullptr
                                               : &input.m_tap_merkle_root;
          uint256 tweak = xonly_pub.ComputeTapTweakHash(merkle_root);
          params.cardTaprootTweakPrivateKeyFn(
              0xff, {tweak.begin(), tweak.end()}, false);
        }
        input.m_tap_key_sig = sign_schnorr_hash(*output_key, hash);
        return true;
      }
      ...
      for (const auto &leaf_hash : leaf_hashes) {
        ...
        input.m_tap_script_sigs[script_sig_key] =
            sign_schnorr_hash(xonly_pub, hash);
      }
    }
    return true;
  };
