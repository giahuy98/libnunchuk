...
void GroupService::StartListenEvents(
  ...
  auto handle_event = [callback =
    ...
    try {
      ...
      ConnectionDebugLog(
          "group-sse",
          strprintf("failed to parse event payload error=%s payload=%s",
                    e.what(), std::string(raw.substr(0, 200)).c_str()));
    }
  };

  if (!stop_) {
    ConnectionDebugLog("group-sse", "restart requested, stopping active stream");
    StopListenEvents();
  }
  ...
  sse_thread_ = std::thread([&, handle_event = std::move(handle_event)] {
    ...
    stop_ = false;
    ConnectionDebugLog("group-sse", "listener thread started");
    while (!stop_) {
      std::string buffer;
      bool received_stream_data = false;
      ConnectionDebugLog("group-sse", "opening SSE stream");
      auto result = sse_client_->Get(
          "/v1.1/shared-wallets/events/sse", headers,
          [&](const httplib::Response& response) {
            ConnectionDebugLog(
                "group-sse",
                strprintf("SSE response status=%d content_type=%s",
                          response.status,
          ...
          [&](const char* data, size_t data_length) {
            if (!received_stream_data) {
              ...
              ConnectionDebugLog(
                  "group-sse",
                  strprintf("received first SSE chunk bytes=%zu", data_length));
            }
      ...
      if (result) {
        ConnectionDebugLog(
            "group-sse",
            strprintf("SSE stream ended status=%d", result->status));
      } else {
        ConnectionDebugLog(
            "group-sse",
            strprintf("SSE stream transport error=%d",
                      static_cast<int>(result.error())));
      }
      if (stop_) break;
      ConnectionDebugLog("group-sse", "retrying SSE stream in 3 seconds");
